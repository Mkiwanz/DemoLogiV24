(function (H) {
    ...
    defaultPlotOptions.numbergauge = merge(defaultPlotOptions.line, {
        ...
        marker: {
            //FIXFOR-V9
            //REPDEV-26535 on mouseOut the graphic will be repositioned based on this radius
            radius: 0
        },
        states: {
            hover: {
    ...
    function cutTitleFont(chart) {
        ...
        styleObj = {
            ...
            fontWeight: chart.options.title.style.fontWeight
        };
    ...
    function scaleTitleFont(chart) {
        ...
        if (chart.options.title.floating) return; //REPDEV-27362. Don't alter (cut) title if it's being floated.
    ...
    function setSpacingGetMaxDimension(axis) {
        ...
        axis.chart.options.chart.spacing[3] = axis.chart.spacing[3];
        ...
        axis.chart.options.chart.spacing[1] = axis.chart.spacing[1];
        var result;
    ...
    //FIXFOR-V9
    //REPDEV-26834 hasData logic changed
    function hasDataGauge () {
        if (this.points)
            return this.points.length;

        var hasData = function (dataArray) {
            return dataArray && dataArray.some(function (dataEntry) {
                return dataEntry || dataEntry === 0;
            })
        };

        return hasData(this.yData) || hasData(this.xData);
    }
    ...
    H.seriesTypes.numbergauge = H.extendClass(H.Series, {
        ...
        animation: null,
        //FIXFOR-V9
        //REPDEV-26879 Replace old mixin or Quick Tip doesn't show
        drawTracker: (H.TrackerMixin && H.TrackerMixin.drawTrackerPoint) || H.seriesTypes.column.prototype.drawTracker,
        trackerGroups: ['group', 'dataLabelsGroup'],
        //FIXFOR-V9
        //REPDEV-26678 Run GaugeNumber throw error
        //REPDEV-26834 hasData logic changed
        hasData: hasDataGauge,
        init: function (chart, options) {
            //FIXFOR-V9
            //REPDEV-26534 HC9: Get rid of old HighchartsAdapter
            (Highcharts.addEvent ? Highcharts : HighchartsAdapter).addEvent(chart, 'setSize', function (e) {
            ...
            //FIXFOR-V9
            //REPDEV-26678 Run GaugeNumber throw error
            this.getExtremesFromAll = function(){ return true; }
        },
        drawPoints: function () {
            var series = this,
            ...
            //FIXFOR-V9
            //REPDEV-26535 on mouseOut the graphic will be repositioned based on the following plotX, plotY
            point.plotX = (targetBox.width / 2) - (sourceBox.width / 2);
            point.plotY = (targetBox.height / 2) + actualBox.baselineShift;
            
            visual.attr({
                x: point.plotX,
                y: point.plotY
            });
            point.tooltipPos = [point.plotX, point.plotY]; // REPDEV-27748
        },
        redraw: function () {
        ...
        getXExtremes: function (xData) {
            var xMin = H.arrayMin(xData), xMax = H.arrayMax(xData);
            //!isNumber
            if (!H.isNumber(xMin)) {
                xMin = arrayMin(xData, true);
            }
            if (!H.isNumber(xMax)) {
                //xMax = arrayMax(xData);
                if (xMax == null) {
            ...
            return {
                min: xMin,//arrayMin(xData),
                max: xMax//arrayMax(xData)
            };
        }
    });
    //FIXFOR-V9
    ...
    H.seriesTypes.arcgauge = H.extendClass(seriesTypes.pie, {
        type: 'arcgauge',
        //FIXFOR-V9
        //REPDEV-26551 : HighChart v9.x: Get js error when create heatmap chart or gauge chart in Info Studio
        //REPDEV-26758 : Highcharts v9-The result of gauge arc chart is not same as v4 when the minimum value is 0
        pointClass: Highcharts.extendClass(seriesTypes.pie.prototype.pointClass, {
            isValid: function () {
                return Highcharts.isNumber(this.y);
            }
        }),
        //FIXFOR-V9
        //REPDEV-26834 hasData logic changed
        ...
        drawPoints: function () {
            ...
            //FIXFOR-V9
            //REPDEV-26551 : HighChart v9.x: Get js error when create heatmap chart or gauge chart in Info Studio
            //point.haloPath = Highcharts.noop;
            //FIXFOR-V9
            //REPDEV-26749 Highcharts v9-Hovering on Gauge Arc chart throw error
            point.haloPath = Highcharts.Point.prototype.haloPath;
            point.graphic = graphic;
            point.tooltipPos = [this.shapeArgs.x, this.shapeArgs.y];//REPDEV-27748
            this.drawLabels();
        },
        ...
        drawLabels: function() {
            ...
            if (showText) {
                ...
                //}).add(graphic);
                var datalabel = renderer.label(labelText, positionX - (nowDim.width / 2), positionY).css({
                    color: this.options.dataLabels.style.color || "#000000",
                    fontSize: this.options.dataLabels.style.fontSize,
                    "font-family": this.options.dataLabels.style.fontFamily,
                    fontWeight: this.options.dataLabels.style.fontWeight
                }).add(graphic);
                if (this.options.dataLabels.className) datalabel.addClass(' ' + this.options.dataLabels.className);
            }
        ...
        translate: function () {
            this.chart.series[0].prevArc && this.chart.series[0].prevArc.destroy();
            this.chart.series[0].prevArc = null;
    ...
    H.seriesTypes.bulletgauge = H.extendClass(seriesTypes.column /*H.Series*/, {//FIXFOR-V9, REPDEV-26498 Gauge bullet bar can not display.
        type: 'bulletgauge',
        animation: null,
        //FIXFOR-V9
        //REPDEV-26879 Replace old mixin or Quick Tip doesn't show
        ...
        init: function (chart, options) {
            ...
            //FIXFOR-V9
            //REPDEV-26534 HC9: Get rid of old HighchartsAdapter
            (Highcharts.addEvent ? Highcharts : HighchartsAdapter).addEvent(chart, 'load', function (e) {
                var axis = chart.xAxis[0];
                scaleTitleFont(chart);
                scaleAxisLabelsFont(axis);
                axis.options.startOnTick = true;
            ...
            //FIXFOR-v9
            //REPDEV-26498 Gauge bullet bar can not display
            chart.xAxis[0].getOffset = customAxisOffset;
        },
        ...
        translate: function () {
            ...
            point.plotY = yAxis.toPixels(yValue) - yAxis.pos;//////??? if plotY > len, annotation at point coordinate can not display
            ...
            point.tooltipPos = [xAxis.toPixels(series.data[0].x) - series.chart.plotLeft, yAxis.toPixels(series.data[0].y) - series.chart.plotTop]; // REPDEV-27748
            //FIXFOR-V9
            //REPDEV-26498 Gauge bullet bar can not display.
            //H.Series.prototype.getSegments.call(this);
            getSegments(this);
    ...
    H.seriesTypes.balloongauge = H.extendClass(H.Series, {
        ...
        tickWidth: 0,
        //FIXFOR-V9
        //REPDEV-26693 HC9: BalloonGauge throws error
        //REPDEV-26834 hasData logic changed
        hasData: hasDataGauge,
        ...
        init: function (chart, options) {
            ...
            chart.xAxis[0].isDirty = true;
            //FIXFOR-v9
            //REPDEV-26972 Gauge BalloonBar has one more tick after resize
            chart.xAxis[0].getOffset = customAxisOffset;
        },
        ...
        translate: function () {
            ...
            point.plotY = yAxis.toPixels(yValue) - yAxis.pos;//point.plotY>yAxis.len, 
        ...
        drawDataLabels: function() {
		    ...
		    dataLabelsGroup;
                ...
                each(points, function (point) {
                    ...
                    series.options.dataLabels.maxLabelLength > 0 &&
                    (str = LogiXML.ChartCanvas.trimLabelToLength ?
                        LogiXML.ChartCanvas.trimLabelToLength(str, series.options.dataLabels.maxLabelLength) :
                        series.trimDataLabels(str)); //LOGIFIX 22191 ChartCanvas: Reverting of an upgrade to v4 framework
                    // Determine the color
                    ...
                    if (dataLabel) {
                        //REPDEV-26693
                        //HC9: BalloonGauge throws error
                        dataLabel.options = { labelrank: 0 };
                        // Now the data label is created and placed at 0,0, so we need to align it
                        series.alignDataLabel(point, dataLabel, options, null, isNew);
                        //REPDEV-27310
                        if (seriesOptions.dataLabels && seriesOptions.dataLabels.className) {
                            dataLabel.addClass(' ' + seriesOptions.dataLabels.className);
                        } else if (seriesOptions.balloonStyle && seriesOptions.balloonStyle.className) {
                            dataLabel.addClass(' ' + seriesOptions.balloonStyle.className);
                        }
                    }
                });
        },
     ...
     * REPDEV-26498 Gauge bullet bar can not display
     * 
     * This will override default getOffset function from HC9
     * except it will not respect tickOffset which will break offset
     * 
     * REPDEV-26816 Obey min and max from options
     * */
    function customAxisOffset() {
        if (this.options && this.options.hasOwnProperty("min"))
            this.min = this.options.min;

        if (this.options && this.options.hasOwnProperty("max"))
            this.max = this.options.max;

        var hasMin = this.hasOwnProperty("min") && !isNaN(this.min);
        var hasMax = this.hasOwnProperty("max") && !isNaN(this.max);

        if (hasMin || hasMax) {
            if (this.tickPositions && this.tickPositions.length) {
                var range = 0;
                if (this.tickInterval)
                    range = Math.floor(this.tickInterval / 2);

                for (var i = this.tickPositions.length - 1; i >= 0; i--) {
                    var tick = this.tickPositions[i];

                    if ((hasMin && (tick - range) <= this.min) || (hasMax && (tick + range) >= this.max))
                        this.tickPositions.splice(i, 1);
                }

                if (hasMin)
                    this.tickPositions.splice(0, 0, this.min);

                if (hasMax)
                    this.tickPositions.push(this.max);
            }
        }
        ...
        //FIXFOR-V9
        //REPDEV-26498 Gauge bullet bar can not display
        //Line below is overrided

        axisOffset[side] = Math.max(axisOffset[side], axis.axisTitleMargin + titleOffset +
            directionFactor * axis.offset, labelOffsetPadded, 0 // #4866
        );
        ...
        Highcharts.fireEvent(this, 'afterGetOffset');
    };
