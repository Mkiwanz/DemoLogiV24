...
var getColorComponent = function (color, offset) {
    ...
    if (typeof (color) != 'string') {
        sColor = color.stops[0][1];
    }
    if (sColor.startsWith("rgb")) {
        return getColorComponentFromRgbString(sColor, offset);
    }
    var bigInt = parseInt(sColor.substring(1), 16);
...
(function (Highcharts) {
    ...
    defaultPlotOptions.treemap = merge(defaultPlotOptions.pie, {
        ...
        //REPDEV-30698, cause <radialGradient cx="NaN" cy="NaN" r="NaN"
        //remarked it. drawDataLabels no longer calls Pie. drawDataLabels
    ...
    seriesTypes.treemap = Highcharts.extendClass(seriesTypes.pie, {
        type: 'treemap',
        //REPDEV-25316, refix (otherwise it will cause REPDEV-30117), the solution used is : (move) sort (to) at set Data.
        setData: function (data, redraw, animation, updatePoints) {
            data && data.sort(function (a, b) { return b.y - a.y });
            Highcharts.Series.prototype.setData.apply(this, arguments);
        },
        //FIXFOR-V9
        //REPDEV-26692 : HC9: using old treemap implementation
        //REPDEV-26802 : Highcharts v9-The rect of heatmap chart is lost when the value lessthan 0
        //               Same reason as REPDEV-26758
        ...
        drawDataLabels: function() {
            ...
            if (options.enabled || series._hasPointLabels) {
                ...
                //REPDEV-27310
                if (series.options.dataLabels.className) dataLabelsGroup.addClass(' ' + series.options.dataLabels.className);
        ...
        translate: function(positions) {
            ...
            var resetPointChildren = function (point) {
                if (point.children && point.children.length > 0) {
                    var mychildren = point.children;
                    point.children = new Array(mychildren.length);
                    for (var index = 0; index < point.children.length; index++) {
                        point.children[index] = Highcharts.extend({}, mychildren[index]);
                    }
                }
            };
            // get the total sum
            var mappingFunction = function (point, nestingLevel) {
                //REPDEV-29751, js error when hovering on
                resetPointChildren(point);
                ...
                //FIXFOR-V9
                //REPDEV-27173 : dashboard heatmap: the color of area becames grey when click on any heatmap plot area
                //Calling setState recursively for children (Group case)
                point.setState = function (state, move) {
                    var currState = state, currMove = move;
                    parentPrototype.setState.call(this, currState, currMove);
                    currState !== 'hover' && this.children.forEach(function (child) {
                        child && child.setState && child.setState(currState, currMove);
                    })
                };
        ...
        alignDataLabel: function (point) {
            ...
            //REPDEV-27748
            point.tooltipPos = [xOffset, yOffset];
        ...
        drawLabel: function (labelText, point) {
            ...
            //FIXFOR-V9
            //REPDEV-26746: (codes 3/3)
            //Bug reason: the empty content label makes overlap.
            //solution1: change getAbsoluteBox() or isIntersectRect() in Chart.prototype.hideOverlappingLabels to skipt empty-content label
            //solution2: do not create empty-content label
            //I choosed solution2
            var dimensions = LogiXML.layout.getTextDimensions(position.text, point.style);
            ...
            //FIXFOR-V9
            //REPDEV-26692 : HC9: using old treemap implementation
            !dataLabel.options && (dataLabel.options = {});
            !dataLabel.options.labelrank && (dataLabel.options.labelrank = 0);
            return dataLabel;
        },
        ...
        drawPoints: function () {
            ...
            var drawPoint = function(point) {
                ...
                //FIXFOR-V9
                //REPDEV-26692 : HC9: using old treemap implementation
                //REPDEV-26781 Highcharts v9 - Hovering on Heatmap chart created on SSRM throw error
                point.haloPath = Highcharts.Point.prototype.haloPath;

                //REPDEV-29276
                point.getNestedProperty = Highcharts.Point.prototype.getNestedProperty;
                //REPDEV-25316, refix FOR HC9 (Maybe it is code mistake on REPDEV-26692)
                ...
                //FIXFOR-V9
                //REPDEV-27173 : dashboard heatmap: the color of area becames grey when click on any heatmap plot area
                //HC9 already respects brightness. Don't apply brightness modifier twice.
