...
(function (H) { // docs
    ...
    function hasDataPie() {
        return !!this.points.length && this.points.some(function (pt) {
            return (!pt.isNull) && (pt.y !== 0);//FIXFOR-V9, REPDEV-26813, Highchart #3571, for NaN; FIXFOR-V9 REPDEV-27152 (restoring pt.y)
        });
    }
    ...
    chartPrototype.showNoData = function (str) {
        ...
        if (!chart.noDataLabel) {
            ...
            chart.legend && chart.legend.update && chart.legend.options.enabled &&
                ((chart.legend.prevState = chart.legend.options.enabled) && chart.legend.update({ enabled: false }, true));
            if (chart.xAxis) {
                for (var i = 0; i < chart.xAxis.length; i++) {
                    chart.xAxis[i].destroy();
                }
                //FIXFOR-V9
                //REPDEV-26939 : HighChart v9.x: there is js error in browser console when run a chart report
                chart.xAxis = [];
                /*if (axis.labelGroup) {
    ...
    chartPrototype.hideNoData = function () {
        ...
        chart.legend && chart.legend.update && !chart.legend.options.enabled && chart.legend.prevState &&
            chart.legend.update({ enabled: chart.legend.prevState }, true);
    };
