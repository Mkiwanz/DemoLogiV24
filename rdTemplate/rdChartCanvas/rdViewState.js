...
var populateLegendVisibilityData = function() {
    ...
    //we could have series instance (non-Pie) and point instance (Pie) here.
    //for point instance hc will revert visibility afterwards so actual visibility should be reverted.
    this instanceof Highcharts.Point && (this.isLegendInvisible = !this.isLegendInvisible);
        ...
        if (chart.series[j].type == "pie") {
            ...
            currentPieSeriesLegendItems.forEach(function (item) {
                Highcharts.addEvent(item, 'legendItemClick', function () {
                        populateLegendVisibilityData.call(this);
                });
            });
        }
    }
...
var setZoomStateEventHandlers = function (chart) {
    ...
    //FIXFOR-V9
    //REPDEV - 26509 : HighChart v9.x CanvasChart with Zoom Control cannot display
    Highcharts.addEvent(chart, 'selection',
        function(event) {
            var xRet = [], yRet = [];
            if (event.xAxis) {
    ...
    for (var i = 0; i < stJson.length; i++) {
        ...
        if (Array.isArray(currentItem)) {
            for (var j = 0; j < currentItem.length; j++) {
                ...
                chart.series[i].data[pieIndex].isLegendInvisible = true;
            }
        }
    } 
};
