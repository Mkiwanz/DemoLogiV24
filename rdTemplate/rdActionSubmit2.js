...
//    }
	...
	if (sConfirm) {
		if (sConfirm.length != 0) {
			LogiXML.confirm(sConfirm, function () {
				this.args[3] = null; // sConfirm
				SubmitForm.apply(this.obj, this.args);
			}.bind({
				obj: this,
				args: arguments
			}));
   ...
   for (var i = 0; i < form.elements.length; i++) {
	...
	if (sPage.search("rdRequestForwarding=Form") == -1) {
        ...
        for (var i = eleInputs.length - 1; i > -1; i--) {
            ...
            if (!eleKeeped.has(eleInput.name)) {
                ...
                // REPDEV-29936 multiple inputs with the same name - remove extraneous rdHiddenRequestForwarding elements
                if (eleInput.id == "rdHiddenRequestForwarding") {
                    // rdHiddenRequestForwarding was set before the page loaded and has less priority.
                    // We already have the other one with the same name.
                    eleRemoved.push(eleInput.parentNode.removeChild(eleInput));
                } else {
                    var savedElem = eleKeeped.get(eleInput.name);
                    if (savedElem.id == "rdHiddenRequestForwarding") {
                        eleRemoved.push(savedElem.parentNode.removeChild(savedElem));
                        eleKeeped.set(eleInput.name, eleInput);
                    } else if (eleInput.type == "hidden") {
                        //we are currently looking on regular input but already have something in keeping bin. Remove current one.
                        //do this only for hidden inputs to remove flickering.
                        eleRemoved.push(eleInput.parentNode.removeChild(eleInput));
                    }
                }
            }
    ...
    if (sPage.indexOf("javascript:")==0) {
        ...
        var sameWindow = (form.target == "_top" || form.target == "_parent" || LogiXML.isSelfTarget(form.target));

        // Show wait panel unless targetting a different window.
        if (waitCfg != null && sameWindow) {
            //25599
...
function NavigateLink2(sUrl, sTarget, bValidate, sFeatures, sConfirm, waitCfg, bFromOnClick) {
	...
	if (sConfirm) {
		if (sConfirm.length != 0) {
			LogiXML.confirm(sConfirm, function () {
				this.args[4] = null; // sConfirm
				NavigateLink2.apply(this.obj, this.args);
			}.bind({
				obj: this,
				args: arguments
			}));
	...
	switch (sTarget) {
		case '_parent':
			...
			break;
		case '_top':
		    LogiXML.layout.getTopWindow().location.href = sUrl;
			break;
		case '_modal':
...
function SubmitFormCrawlerFriendly(sPage, sTarget, bValidate, sConfirm, waitCfg) {
    sPage = unescape(sPage);
    SubmitForm(sPage, sTarget, bValidate, sConfirm, false, waitCfg);
}

function NavigateCrawlerFriendly(sUrl, sTarget, bValidate, sFeatures, sConfirm, waitCfg) {
    sUrl = unescape(sUrl);
    NavigateLink2(sUrl, sTarget, bValidate, sFeatures, sConfirm, waitCfg);
}
