    ...
    Y.namespace('LogiXML').ReportAuthor = Y.Base.create('ReportAuthor', Y.Base, [], {
        ...
        addVisualizationsPopupDone: function () {
            ...
            if (visualizationIds && visualizationIds.length > 0) {
                var cmdParams = 'rdAjaxCommand=RefreshElement&rdRefreshElementID=' + this.id + ',' + refreshElementId +
                    '&rdReportAuthorAction=refresh&rdReportAuthorElementID=' + refreshElementId + '&rdReport=' + this.reportName;
                var eleRefreshDataCache = document.getElementById('rdRefreshDataCache');
                if (eleRefreshDataCache && eleRefreshDataCache.value) {
                    cmdParams = cmdParams + '&rdRefreshDataCache=' + eleRefreshDataCache.value;
                }

                rdAjaxRequestWithFormVars(cmdParams, 'false', '', null, null, null, ['', '', '']);
