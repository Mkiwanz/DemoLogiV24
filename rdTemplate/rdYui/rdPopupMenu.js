	...
	Y.namespace('LogiXML').PopupMenu = Y.Base.create('popupmenu', Y.Base, [], {
		...
		initializePopupMenus: function () {
			...
			menulinks.each( function(initLink) {			
				...
				// if old menu exists, remove it so it can be rebuilt later in rdShowPopupMenu
				var menu = YAHOO.widget.MenuManager.getMenu(sPopupId);
				if (menu) {
					menu.hide(menu);
		...
		rdShowPopupMenu: function(sMenuId, sPopupLocation) {
            ...
            var ul = popup.all("UL");           
			if (ul) {
				ul.each(function (_ul) { // for each ul inside popup
					_ul.get('childNodes').each(function (_child) { // for each child of _ul
						// expecting li elements, but some are wrapped in rcond ele
						if (_child._node && _child._node.nodeName && _child._node.nodeName.toLowerCase().indexOf("rdcondelement") >= 0) {
							_child.get('childNodes').each(function (_moveUp) {
								_ul._node.insertBefore(_moveUp._node, _child._node);
								_ul._node.removeChild(_child._node);
							});
						}
					});
				});
