...
YUI.add('chartDrillToBreadcrumb', function (Y) {
    ...
    Y.namespace('LogiXML').ChartDrillToBreadcrumb = Y.Base.create('ChartDrillToBreadcrumb', Y.Base, [], {
        ...
        initializer: function (config) {
            ...
            this.isSsrmDashboard = config.isSsrmDashboard;
            this._handlers = {};
            this.render();
        },
        render: function () {
            ...
            if (this.drillFilterCaptionContainer) {
                ...
                if (drilledToStates && drilledToStates.length > 0) {
                    ...
                    if (!this.rdChartDrillPanel) {
                        if (this.isSsrmDashboard == 'True') {
                            this.rdChartDrillPanel = Y.Node.create('<ul class="rdChartDrillPanel"></ul>');
                        } else {
                            this.rdChartDrillPanel = Y.Node.create('<ul class="rdChartDrillPanel" style="padding-inline-start: 0px;"></ul>');
                        }
        ...
        reflow: function () {
            ...
            var containerWidth = Number.MAX_SAFE_INTEGER;
            ...
            if (dashPanel) {
                ...
                containerWidth = bodyPanel.getDOMNode().offsetWidth - clearBtn.getDOMNode().offsetWidth;
            } else {
                var divChartContainer = Y.one('div.rdChartCanvas#' + this.rdChartCanvasId);
                var rect = divChartContainer && divChartContainer.one("rect.highcharts-plot-background");
                if (rect) {
                    var clearBtnWidth = clearBtn? clearBtn.getDOMNode().offsetWidth: 0;
                    var rectWidth = rect.getDOMNode().width.animVal.value
                    containerWidth = rectWidth - clearBtnWidth;
                }
            }
