...
function rdSchSave(schId,eleId) {
    ...
    if (sValidate != null && sValidate != "") {
        ...
        if (sNewValue != null) { LogiXML.alert(sValidate + "\n\nValue: " + sNewValue); }
        else { LogiXML.alert(sValidate); }
        return "";
    }
    else {
        ...
        var sXml = "<Schedule Type=\"" + (sInterval == "Monthly" ? (document.getElementById("chkSchMonthLastDay_" + schId + "_1").checked ? "MonthlyLastDay" : (document.getElementById("chkSchMonthDayInterval_" + schId + "_1").checked ? "MonthlyDayOfWeek" : sInterval)) : sInterval) + "\"";
        ...
        // Set the timezone
        sXml += " TimezoneID=\"" + document.getElementById("selSchTimezone_" + schId).value + "\"";
        ...
        switch (sInterval) {
            ...
            case "Monthly":
                ...
                } else if (document.getElementById("chkSchMonthLastDay_" + schId + "_1").checked) {
                    //do nothing except setting "type" if the last day of month is selected
                } else {
...
function rdSchAddParam(schId, paramName) {
    // Prompt the user for a name for the new parameter.
    if (typeof paramName === "undefined") {
        LogiXML.prompt("Please enter a name for the parameter.", "", function (value) {
            rdSchAddParam(this.schId, value); //REPDEV-29744
        }.bind({
           schId: schId
        }));
        return;
...
function rdSchUpdateControls(schId) {
    ...
    if (sInterval != "Monthly") {
        ...
        ShowElement(this.id, "rowSchLastMonthDay_" + schId, "Hide");
        ...
        if (document.getElementById("rowSchLastMonthDay_" + schId).attributes["class"] == null ||
            document.getElementById("rowSchLastMonthDay_" + schId).attributes["class"].value != "rdSchHidden") {
            ShowElement(this.id, "rowSchLastMonthDay_" + schId, "Show");
        }
...
function rdSchToggleMonthlyInterval(toggle, schId) {
    switch (toggle) {
        case "day":
            ShowElement(this.id, "divSchMonths_" + schId, "Show");
            ShowElement(this.id, "divSchMonthDayIntervalPlaceHolder_" + schId, "Show");

            ShowElement(this.id, "divSchMonthDayInterval_" + schId, "Hide");
            ShowElement(this.id, "divSchMonthsPlaceHolder_" + schId, "Hide");

            document.getElementById("chkSchMonths_" + schId + "_1").checked = true;
            document.getElementById("chkSchMonthLastDay_" + schId + "_1").checked = false;
            document.getElementById("chkSchMonthDayInterval_" + schId + "_1").checked = false;
            break;
        case "lastDay":
            ShowElement(this.id, "divSchMonths_" + schId, "Hide");
            ShowElement(this.id, "divSchMonthDayIntervalPlaceHolder_" + schId, "Show");

            ShowElement(this.id, "divSchMonthDayInterval_" + schId, "Hide");
            ShowElement(this.id, "divSchMonthsPlaceHolder_" + schId, "Show");

            document.getElementById("chkSchMonths_" + schId + "_1").checked = false;
            document.getElementById("chkSchMonthLastDay_" + schId + "_1").checked = true;
            document.getElementById("chkSchMonthDayInterval_" + schId + "_1").checked = false;
            break;
        default:
            //"variable": week day of week
            ShowElement(this.id, "divSchMonths_" + schId, "Hide");
            ShowElement(this.id, "divSchMonthDayIntervalPlaceHolder_" + schId, "Hide");

            ShowElement(this.id, "divSchMonthDayInterval_" + schId, "Show");
            ShowElement(this.id, "divSchMonthsPlaceHolder_" + schId, "Show");

            document.getElementById("chkSchMonths_" + schId + "_1").checked = false;
            document.getElementById("chkSchMonthLastDay_" + schId + "_1").checked = false;
            document.getElementById("chkSchMonthDayInterval_" + schId + "_1").checked = true;
    }
