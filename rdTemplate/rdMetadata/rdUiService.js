    ...
    if (eleInput) {
        ...
        if (sCommand == 'TableSetValue' && eleInput.id != 'inputSqlSource') {
            if (sValue.indexOf('\'') >= 0 || sValue.indexOf('"') >= 0) {
                LogiXML.alert("Table names cannot contain any quotes, single or double.  Please try again.");
...
function rdMetadataSortColumn(sElementID, sDataCacheKey, sColumnName, sDataType, descending, sParams) {
    var sUrl = "rdRequestForwarding=Form&rdReport=rdTemplate/rdMetadata/Columns"
        + "&rdDataCache=" + encodeURIComponent(sDataCacheKey)
        + "&rdSort=" + sElementID + "~" + sColumnName + "~" + sDataType + "~" + (descending ? "Descending" : "Ascending") + "~" + (descending ? "Ascending" : "Descending")
        + "&rdRememberSort=True";
...
function rdMetadataCustFriendlyName(sCustFriendlyNameType, sDataCacheKey, sParams) {
    var sUrl = "rdRequestForwarding=Form&rdReport=rdTemplate/rdMetadata/Columns"
        + "&rdDataCache=" + encodeURIComponent(sDataCacheKey)
        + "&rdCustFriendlyNameType=" + sCustFriendlyNameType;
...
function rdMetadataShowTableEdit(sMetadataID, sConnectionID) {
    ...
    sUrl += "&TableKey=" + encodeURIComponent(rdMetadataReturnedTableName)
    NavigateLink2(sUrl, "_self")
}
