    ...
    if (aMapPolylineRows.length > 0) {
        for (var i = 0; i < aMapPolylineRows.length; i++) {
            ...
            if (encodedPoints.length > 0) {
                ...
                // REPDEV-28528 Google polylines need an array of obj with lat and lng
                // not an array of arrays of obj with lat and lng as with polygons
                if (polyPaths.length == 1 && Array.isArray(polyPaths[0]))
                    polyPaths = polyPaths[0];
