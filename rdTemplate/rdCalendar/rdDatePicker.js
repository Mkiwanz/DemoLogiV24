	...
	function rdInsertDates(sStartDateElementID, sEndDateElementID, sParamDate, nDay, sDisplayDate, nCalendarNo, sElementIdentifier, sRowIdentifier){
        rdGetRowIdentifier(sRowIdentifier);
	    ...
	    if(!(rdCalendarClicks % 2)){
	        if(document.getElementById(sStartDateElementID)== null){
                LogiXML.alert('Could not find the Start Date Element or the provided Element is not appropriate.');
                return;
            }
...
//            }
	        ...
	        if(document.getElementById(sEndDateElementID)== null){
                 LogiXML.alert('Could not find the End Date Element or the provided Element is not appropriate.');
                 return;
            }
             document.getElementById(sEndDateElementID).value = sDisplayDate;
             if(document.getElementById(sEndDateElementID).onchange)
    ...
    function rdInsertSingleDate(sStartDateElementID, sPopUpID, sParamDate, nDay, sDisplayDate, nCalendarNo, sElementIdentifier, sRowIdentifier){
        rdGetRowIdentifier(sRowIdentifier);
       ...
       if(rdCalendarRowIdentifier == undefined){
        ...
        if (!document.getElementById(sStartDateElementID)) {
            sStartDateElementID = sStartDateElementID.substr(0, sStartDateElementID.length - rdCalendarRowIdentifier.length);
            if (!document.getElementById(sStartDateElementID)) {
                LogiXML.alert('Could not find the Target Date Element or the provided Element is not appropriate.');
                return;
            }
            // row id is from a different date picker, one that exists in a repeater, whereas this one does not
            rdGetRowIdentifier("");
            sHiddenDateHolderElement = sStartDateElementID;
        }
...
//        }    
    ...
    function rdOnLoadJavascriptAddition(sElementIdentifier){
        ...
        for (y=1; y<=3; y=y+1){
            ...
            for(x=1; x<=31; x=x+1){
                (function(x)
                {
                    var sElement = rdGetElement(x, y, sElementIdentifier);

                    if (sElement != null) {
                        found = true;
                        sElement = sElement.parentNode;
        ...
        if (found) {
            ...
            rdDatePicker_InitAllRows(sElementIdentifier, rdOnLoadJavascriptAddition);
        }
    }
    ...
    function rdGetElement(sNumber, nCalendarNo, sElementIdentifier ){
        ...
        if (ele == null && rdCalendarRowIdentifier) {
            var i = sElementIdentifier.lastIndexOf(rdCalendarRowIdentifier);
            if (i == sElementIdentifier.length - rdCalendarRowIdentifier.length)
                return rdGetElement(sNumber, nCalendarNo, sElementIdentifier.substr(0, i));
        }
        return ele;
    }
    ...
    function rdInputDateAjaxRequest(sInputDateID, sDatePickerID, sElementID, sReportID, sRowIdentifier){
        rdGetRowIdentifier(sRowIdentifier);
...
//                sDatePickerID = sDatePickerID + rdCalendarRowIdentifier; //#14844.           
         ...
         var sParms = 'rdAjaxCommand=CalendarRefreshElement' +
             '&rdCalendarRefreshElementID=' + sInputDateID + ',' + sDatePickerID +
             '&rdInputDateValue=' + document.getElementById(sElementID).value +
             '&rdReport=' + sReportID;

         if (rdCalendarRowIdentifier)
             sParms += '&rdCalendarRowIdentifier=' + rdCalendarRowIdentifier;
    ...
    function rdGetRowIdentifier(sRowIdentifiervalue){
        if(sRowIdentifiervalue == undefined) 
            ...
            rdCalendarRowIdentifier = '_Row' + String(sRowIdentifiervalue).replace(/[^\d]/g, '');
    }
...
function rdDatePicker_ApplyRowIdToRefreshElement(newEle) {
    if (!rdCalendarRowIdentifier)
        return newEle;

    var id = newEle.getAttribute("id");
    var nameToo = (newEle.getAttribute("name") == id);
    if (id && id.lastIndexOf(rdCalendarRowIdentifier) != id.length - rdCalendarRowIdentifier.length) {
        id += rdCalendarRowIdentifier;
    ...
    for (var i = 0; i < newEle.childNodes.length; i++) {
        ...
        if (child.nodeType == 1)
            rdDatePicker_ApplyRowIdToRefreshElement(child);
    }

    return newEle;
}

function rdDatePicker_InitAllRows(sElementIdentifier, fx) {
    if (rdCalendarRowIdentifier || rdGetElement(1, 1, sElementIdentifier))
        return;

    var initialRowIdentifier = rdCalendarRowIdentifier;
    var sElementModifier = rdElementModifier(1, sElementIdentifier);
    var sIdPrefix = sElementModifier + 'rdCalDay_1_DayTable_Row';
    var loadRows = document.querySelectorAll("[id^=" + sIdPrefix + "]");
    var rowIds = [];
    for (var i = 0; i < loadRows.length; i++) {
        var id = loadRows[i].getAttribute("id");
        var row = Number(id.substr(id.lastIndexOf("_Row") + 4));
        if (!isNaN(row)) {
            rdGetRowIdentifier(row);
            fx(sElementIdentifier);
        }
    }
    rdGetRowIdentifier(initialRowIdentifier);
}
