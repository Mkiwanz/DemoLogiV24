...
function rdAjaxRequest(commandParams, bValidate, sConfirm, bProcess, fnCallback, waitCfg, isUpload) {
   ...
   if (sConfirm) {
		if (sConfirm.length != 0) {
			LogiXML.confirm(sConfirm, function () {
				this.args[2] = null; // sConfirm=null
				rdAjaxRequest.apply(this.obj, this.args);
			}.bind({
				obj: this,
				args: arguments
			}));
    ...
    if (waitCfg != null) {
        ...
        if (Y.Lang.isValue(LogiXML.WaitPanel.pageWaitPanel)) {
            ...
            var iTimeout = 500;
            if (waitCfg.timeout) iTimeout = waitCfg.timeout;//REPDEV-28806
            tTimeout = new Timeout(function () { LogiXML.WaitPanel.pageWaitPanel.showWaitPanel(waitCfg) }, iTimeout);
        }
	}
...
function resizeCurrentIframe() {
    try {
        ...
        // parent is likely in a different domain and/or not a Logi application
    }
}
    ...
    if (sResponse.length != 0) {
	    ...
	    switch (xmlResponse.documentElement.getAttribute("rdAjaxCommand")) {
		        ...
		        case 'RefreshElement':
				    ...
				    if (isDataTablePaging && isDataTablePaging.indexOf("rdDataTableDiv") > -1) {
				        ...
				        if (eleTableDiv) {
				            ...
				            var sScrollID = sTableDivID.substring(sTableDivID.indexOf("rdDataTableDiv") + 15);//rdDataTableDiv-
				            var isAG = false;
				            if (sScrollID == "dtAnalysisGrid") {
					            isAG = true;
				            }
				            sScrollID = sScrollID + "_scrolling";
				            var oldScrollDiv = eleTableDiv.querySelector("DIV#" + sScrollID);
				            var oldScrollTop = oldScrollDiv && oldScrollDiv.scrollTop;
				            replaceHTMLElement(eleTableDiv, sResponse, eleTableDiv.id);
				            /*
				             * NOTE: InitScrollAreaAfterAjaxRefresh is used to 'set window's resize listener' and 'position the slider'.
				             * On 'refreshed_' or 'reinitialize', there is no oldScrollTop(as it was destoried in 'replaceHTMLElement'), 
				             *      which causes the slider to go back to the top(so that the always jump to show the first page).
				             * AND, If ResizeableColumn is used, many actions(such as Move/resize Column, Add to gallary(on AG), Hide/Show (on AG)) will trigger 'reinitialize', 
				             *      so in addition to the slider positioning problem, it will also cause WindowsResizeListener constantly being added
				             * 3rd, 'set widnow's resize listener' is for REPDEV-28605, if there is other solution for it, only need to do 'position the slider' here
				             * 4th,  we can put 'position the slider' here, and move 'set window's resize listener' to "refreshed_". 
				             *       -- that means we need add 'refreshed_TableID" listener on rdInteractivePaging.AttachAutoScrolling()
				             */
				            Y.LogiXML.rdInteractivePaging && Y.Lang.isFunction(Y.LogiXML.rdInteractivePaging.InitScrollAreaAfterAjaxRefresh) && Y.LogiXML.rdInteractivePaging.InitScrollAreaAfterAjaxRefresh(document.getElementById(sScrollID), oldScrollTop );
				            //REPDEV-29968 Custom option below Aggregate doesn't display when click Next/Last page button
				            //bind the (custom) menu to the table header.
				            if (isAG) {
				...
				case 'CalendarRefreshElement':  // Block added to support the Ajax refresh for the calendar element
					...
					var rowType = xmlResponse.documentElement.getAttribute('rdCalendarRowIdentifier');
					if (rowType) {
						if (rdGetRowIdentifier)
							rdGetRowIdentifier(rowType);
						rowType = "date";
					} else {
						rowType = xmlResponse.documentElement.getAttribute('rdInputTimeRowIdentifier');
						if (rowType) {
							if (rdGetInputTimeRowIdentifier)
                                rdGetInputTimeRowIdentifier(rowType);
							rowType = "time";
						}
					}
					...
					for (var i=0; i <  sElementIDs.length; i++) { 						
						...
						switch (rowType) {
							case "date":
								eleOld = document.getElementById(sElementID + rdCalendarRowIdentifier);
								break;
							case "time":
								eleOld = document.getElementById(sElementID + rdInputTimeRowIdentifier);
								break;
							default:
						...
						if (eleOld) {
							...
							if(sNewHtml){
								...
								switch (rowType) {
									case "date":
										if (rdDatePicker_ApplyRowIdToRefreshElement) {
											rdDatePicker_ApplyRowIdToRefreshElement(eleNew);
										}
										break;
									case "time":
										if (rdTimePicker_ApplyRowIdToRefreshElement) {
											rdTimePicker_ApplyRowIdToRefreshElement(eleNew);
										}
										break;
								}
            ...
            case 'ShowStatus':
                ...
                if (xmlResponse.documentElement.getAttribute("Alert")) {
		            LogiXML.alert(xmlResponse.documentElement.getAttribute("Alert"));
		        }
...
function replaceHTMLElement(eleOld, sResponse, newElementID) {
	...
	if (oldYuiNode)
        //after rdserverhtmlfixup, the attributes are changed to lowercase
    ...
    while (nDepth > 0) {
        ...
        if (nNextEnd != -1 && nNextEnd < nNextStart) {
            ...
            LogiXML.alert('There was an error parsing the returned Ajax XML.'); //todo, keep this?  Throw an error instead.
            return null; //error.
        }
    }
    var nEleEnd
...
function rdNestedConfirm(sConfirm, onOk, onCancel) {
    var ele = document.getElementById('lblBookmarkCaption');
    
    if (ele)
        sConfirm = 'Duplicate "' + ele.innerHTML + '"?';

    return LogiXML.confirm(sConfirm, onOk, onCancel);
}

function rdExcelPasswordPopup_AfterRefresh(actionbtnId) {
    var ele = document.getElementById(actionbtnId);
    if (ele != null) {
        ele.click();
    }
}