...
YUI.add('dashboard', function (Y) {
    ...
    Y.namespace('LogiInfo').Dashboard = Y.Base.create('Dashboard', Y.Base, [], {
        ...
        initializer: function (config) {
            ...
            //REPDEV-30079, if sorting, reinitialize, too.
            if (!reinitializing && LogiXML.Ajax.AjaxTarget) {
        ...
        rdAddDashboardPanel: function (sPanelID, nRowNr, eleEventOriginationPopup) {
            //suppose the user will modify the html to show the hiden "Add" button and then click it to add the panel.
            //So check the state first and then add it.
            var limitState = this.rdCheckPanelLimitState();
            if (limitState[0]) {
                return;
            }
            var bDisableAdded = limitState[1];
            ...
            if (typeof eleEventOriginationPopup === 'undefined') {
                ...
                // //REPDEV-28528 The display is not very well.
                if (bDisableAdded) {
                    this.rdDisableAddButton();
                }
        ...
        //Returns true if the number of panels has reached the limit.
        rdCheckPanelLimitState: function () {
            var eleLimit = document.getElementById("rdDashboardVSLimit");
            var dsLimit = (eleLimit && parseInt(eleLimit.value)) || 0;
            eleLimit = document.getElementById("rdTabVSLimit");
            var tabLimit = (eleLimit && parseInt(eleLimit.value)) || 0;
            var retState = [false, false];
            if (dsLimit <= 0 && tabLimit <= 0) {
                return retState;
            }
            var bAddPanel = true;
            var eleDSVSTotal,eleTbVsTotal;
            var count_DSVS, count_TbVS;

            //check whether a panel can be added to the dashboard
            if (dsLimit > 0) {
                eleDSVSTotal = document.getElementById("rdDashboardVSTotal");
                count_DSVS = parseInt(eleDSVSTotal.value);
                bAddPanel = count_DSVS < dsLimit;
            }

            if (tabLimit > 0) {
                eleTbVsTotal = document.getElementById("rdTabVSTotal");
                count_TbVS = parseInt(eleTbVsTotal.value);
                bAddPanel = bAddPanel && count_TbVS < tabLimit;
            }
            bDisableAdded = !bAddPanel;
            //suppose the use will modify the shown panels number, so we correct the shown values in here, even if the panel cannot add to the dashboard.
            if (dsLimit > 0) {
                var eleCount = document.getElementById("rdDashboardVSTotalShown");
                var eleLimit = document.getElementById("rdDashboardVSLimitShown");
                if (bAddPanel) {
                    count_DSVS++;
                    eleDSVSTotal.value = count_DSVS;
                    bDisableAdded = count_DSVS >= dsLimit;
                }
                eleCount.innerHTML = count_DSVS;
                eleLimit.innerHTML = dsLimit;
            }

            if (tabLimit > 0) {
                var eleCount = document.getElementById("rdTabVSTotalShown");
                var eleLimit = document.getElementById("rdTabVSLimitShown");
                if (bAddPanel) {
                    count_TbVS++;
                    eleTbVsTotal.value = count_TbVS;
                    bDisableAdded = bDisableAdded || count_TbVS >= tabLimit;
                }
                eleCount.innerHTML = count_TbVS;
                eleLimit.innerHTML = tabLimit;
            }
            retState[0] = !bAddPanel;
            retState[1] = bDisableAdded;
            return retState;
        },

        rdDisableAddButton: function () {
            var allEleAddPanels = document.getElementById("ppChangeDashboard").querySelectorAll("span[id^='lblAddPanel_Row']");
            var allEleDisableAddPanels = document.getElementById("ppChangeDashboard").querySelectorAll("span[id^='lblDisableAddPanel_Row']");
            for (i = 0; i < allEleAddPanels.length; i++) {
                var eleAddPanel = allEleAddPanels[i];
                eleAddPanel.className = "rdDashboardHidden";
            }
            for (i = 0; i < allEleDisableAddPanels.length; i++) {
                var eleDisableAddPanel = allEleDisableAddPanels[i];
                var sAddId = eleDisableAddPanel.getAttribute("id");
                var sHiddenId = sAddId.replace("lblDisableAddPanel_", "hiddenMultiInstance_");
                if (document.getElementById(sHiddenId).value == "False") {
                    var eleCount = document.getElementById(sAddId.replace("lblDisableAddPanel_", "lblCount_"));
                    var countValue = parseInt(eleCount.innerHTML)
                    if (countValue == 0) {
                        eleDisableAddPanel.className = "rdDisableDashboardCommand";
                    } else {
                        eleDisableAddPanel.className = "rdDashboardHidden";
                    }
                } else {
                    eleDisableAddPanel.className = "rdDisableDashboardCommand";
                }
            ...
            if (!Y.Lang.isNull(nodeActiveTab)) {
                ...
                if (typeof (rdMobileReport) == 'undefined') {
                    ...
                    if (Y.Lang.isNull(nodeAddNewTabCog)) {
                        ...
                        //REPDEV-30079
                        if (nodeSettingsCog.getDOMNode().childNodes.length == 1){
                            nodeSettingsCog.setStyles({
                                display: 'none'
                            });
                        }
        ...
        rdMoveMobileDashboardPanel: function (sDashboardPanelId, sChangePanelAction, sPopupPanelID) {
            ...
            switch (sChangePanelAction) {
                ...
                case "Edit":
                    ...
                    break;
                case "Remove":
                    LogiXML.confirm("Are you sure?", function () {
                        this.obj.rdRemoveDashboardPanel(this.sDashboardPanelId);
                    }.bind({
                        obj: this,
                        sDashboardPanelId: sDashboardPanelId
                    }));
                    break;
                default:
		...
		//REPDEV-27386
		rdPopupPanelFilterPanel: function (panelID, panelInstID, sReport) {
			var subfix = panelID + "_" + panelInstID;
			var doPopup = function () {
				//Copied from Template
				ShowElement(null, "ppPanelParams-" + subfix, 'Show');
				LogiXML.Dashboard.pageDashboard.rdAddRefreshEventForPanelEditPopupCloseButton("rdDashboardPanel-" + subfix, '');
			};

			var ele = document.getElementById("rdPanelFilterCreated-" + subfix)
			if (ele && ele.value == "False") {
				//Note: In my original design1, 'rdAnalysisFilterID' is used but not 'rdGlobalFilter','rdPanelID' & 'rdInstanceID', that is only process the PanelFilter or GlobalFilter that equals it
				//But, because PanelFilter only need to process one at a time, I changed to 'rdGlobalFilter=False' and 'rdPanelID' for quick return.
				//But, because the 19625 and 19773 in rdserver.vb, unnecessary panels have been deleted (see '19625 19773 marked in rdserver), so ONLY keep 'rdGlobalFilter' and 'rdInstanceID'
				//Note: my backup design is that uses 'rdRefreshDashboard=True', 'rdPanelID', 'rdInstanceID', 'rdGlobalFilter=False' & 'rdRefreshElementID=ppPanelParams-' for PanelFilter
				var reqStr = "rdAjaxCommand=RefreshElement&rdAfCommand=Initialize&rdGlobalFilter=False";
				reqStr += "&rdRefreshElementID=ppPanelParams-" + subfix + "," + panelID;
				reqStr += "&rdInstanceID=" + panelInstID
				reqStr += "&rdReport=" + sReport;
				rdAjaxRequest(reqStr, 'false', '', false, doPopup, ['', '', ''], false)
			} else {
				doPopup();
			}
		},
		//REPDEV-27701
		rdShowGlobalFilterPanel: function (sReport) {
			var doPopup = function () {
				document.getElementById('rdGlobalFilterVisible').value = 'True';
				ShowElement(this.id, 'rdGlobalAfContainer', 'Toggle', 'FadeIn');
				ShowElement(this.id, 'rowGlobalFilterCaption', 'Hide');
				Y.LogiXML.ChartCanvas.reflowAllCharts();
			}
			var ele = document.getElementById("rdGlobalFilterInitialized");
			if (ele && ele.value == "False") {
				//Note: In my original design1, 'rdAnalysisFilterID' is used but not 'rdGlobalFilter' and 'rdTabId', that is only process the GlobalFilter that equals it
				//But, because there is no ( or complex logic to find) sTabId on eleDef/elePanel, I changed to 'rdGlobalFilter=True' and 'rdRefreshDashboard=True' for quick return.
				var reqStr = "rdAjaxCommand=RefreshElement&rdAfCommand=Initialize&rdGlobalFilter=True";
				reqStr += "&rdRefreshElementID=rdGlobalAfContainer";
				reqStr += "&rdRefreshDashboard=True";
				reqStr += "&rdReport=" + sReport;
				rdAjaxRequest(reqStr, 'false', '', false, doPopup, ['', '', ''], false)
			} else {
				doPopup();
...
function rdSaveEditedAgViz(sReport, sVizType, sPanelID, sInstanceID, sBookmarkID, sDraggableColumnsID) {
    if (!sPanelID)
    ...
    switch (sVizType) {
        ...
        case "AxTable":
            ...
            rdPanelParams += "&rdDraggableColumnsID=" + sDraggableColumnsID;
            break;
        case "AgTable":
...
function rdDbEnableGlobalFilterHighlighting() {
    ...
    //REPDEV-27277, Because Now DesignModel does not have Enable/Disable CheckBox. so only use Current-CaptionLine to skip the disabled
    //Note1: sGlobalFilterCaptionLine also can be get in SimpleModel.
    //Note2: Maybe we need have Enable/Disable checkbox.
    var sGlobalFilterCaptionLine = undefined;
    if (listGlobalFilterCheckBoxes.size() == 0 && listGlobaFilterDataColumns.size() > 0 ) {//Design Model && (at least) 1 filter. 
        var sAfId = listGlobaFilterDataColumns.item(0)._node.id.substring(20); //remove 'lblFilterDataColumn_'
        sAfId = sAfId.substring( 0, sAfId.lastIndexOf("_") ); //remove "_rowX"
        sGlobalFilterCaptionLine = document.getElementById("rdAfCaptionLine_" + sAfId).textContent;
    }
    ...
    for (var i = 0; i < listGlobaFilterDataColumns.size() ; i++) {
        ...
        bSpanAlreadyExists = rdDoesSpanExist(eleFilterCaption, listCurrentSpanNodes)
                ...
                if (elePanelFilterColumns) {
                    ...
                    if (aPanelColumns.indexOf(sDataColumn) != -1) {
                        if (sFilterFromPanelID == "" || elePanel.id.indexOf(sFilterFromPanelID) == -1) {
                            ...
                            if (sGlobalFilterCaptionLine == undefined || sGlobalFilterCaptionLine.indexOf(sFilterCaption) >= 0) {//REPDEV-27277,(comment are above)
...
function rdDoesSpanExist(CurrentFilterCaptionNode, listCurrentSpanNodes) {
    sCurrentFilterCaption = CurrentFilterCaptionNode.innerText;
    for (var i = 0; i < listCurrentSpanNodes.length ; i++) {
        if (listCurrentSpanNodes[i].getAttribute('id') == "rdGlobalFilterCaption") {
            continue;
        }
        //REPDEV-30048
        var sSpanColumn = listCurrentSpanNodes[i].getAttribute('rdfiltercolumn');
        var sCurrentColumn = CurrentFilterCaptionNode.getAttribute('rdfiltercolumn')
        if (sSpanColumn != sCurrentColumn) {
            continue;
        }
...
function rdShowAddGalleryPanels(id, report) {
    ...
    // REPDEV-27031 Commenting this calculation snippet as this is not being called on find key up event, after the performance changes(REPDEV-22546).
    // Added CSS property in rdDashboard2.css for the same.
