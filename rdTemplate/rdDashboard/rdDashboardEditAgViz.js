...
function rdSaveStandaloneEditedAgVis(sReport, sDashReport, sVizContainerID, sBookmarkID, sBookmarkCollection, sVizType, sDraggableColumnsID) {
    ...
    switch (sVizType) {
        ...
        case "AxTable":
            ...
            adjReqString += "&rdDraggableColumnsID=" + sDraggableColumnsID;
            break;
        case "AgTable":
    ...
    adjReqString += "' , 'false', '', null, null, fCallback, ['', '', ''], true)";

    //REPDEV-28806, make the second request be done after the first request
    //eval(adjReqString); //updating panel
    //adjReqString = adjReqString.replace(("&rdDraggableColumnsID=" + sDraggableColumnsID), "");
    ...
    var adjReqString1 = adjReqString.replace(("&rdDraggableColumnsID=" + sDraggableColumnsID), "");
    adjReqString1 = adjReqString1.replace(sDashReport, sReport);
    adjReqString1 = adjReqString1.replace("&rdCommand=AddDashboardPanel", "");
    adjReqString1 = adjReqString1.replace("&rdAgAddToDashboard=True", "");
    adjReqString1 = adjReqString1.replace("fCallback", "null");

    var afterSave = function () {
        window.parent.eval(this.sCommand);//updating original report
    }.bind({
        sCommand: adjReqString1
    });
    adjReqString = adjReqString.replace("fCallback", "afterSave")
    eval(adjReqString); //updating panel
