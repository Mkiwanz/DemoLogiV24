	...
	Y.namespace('LogiInfo').AnalysisGrid = Y.Base.create('AnalysisGrid', Y.Base, [], {
		...
		initializer : function() {
			...
			if (!Y.Lang.isValue(Y.one('hideAgMenu'))
				...
				var sTabName = document.getElementById('rdAgCurrentOpenTablePanel').value;
				if (sTabName == "Aggr") {
					this.rdAgShowTableMenuTab(sTabName, true, '', false);
				} else {
					this.rdAgShowTableMenuTab(sTabName, true);
                }				
            }
		...
		rdChangeAggregateOptions: function(){
            ...
            rdchangeList('rdAgAggrFunction', aAggrList, aAggrListLabel, dataType, '', '', this.CURRENT_COLUMN_TOKEN);//REPDEV-29844,pass CURRENT_COLUMN_TOKEN in.
		...
		rdAgShowTableMenuTab: function (sTabName, bCheckForReset, sSelectedColumn, bNotKeepMenuStatus) {

			var bNoData = false;
			var bOpen = true;
			if (bNotKeepMenuStatus==undefined) {
				bNotKeepMenuStatus = true
            }
		...
		selectPropertiesTab: function (sTabName) {
			var eleSelectedTab = document.getElementById('lbl' + sTabName);
			var eleSelectedRow = document.getElementById('row' + sTabName);
			this.rdSetClassNameById('lblCaptionProperties', 'rdAgCommandIdle');
			this.rdSetClassNameById('lblLabelProperties', 'rdAgCommandIdle');

			this.rdSetDisplayById('rowCaptionProperties', 'none');
			this.rdSetDisplayById('rowLabelProperties', 'none');

			if (eleSelectedTab) {
				document.getElementById('rdAcAxisSettingCurrentTab').value = sTabName;
				eleSelectedTab.className = 'rdAgCommandHighlight';
			...
			//Below code for fixing the issue:Editting the input value of the Datalabel color only can work for the first time. 
			//That is, the second editting on the color input can not work. (note: Caption font color does not this issue.)
			//It will cause the issue: 28376, so rollback it
			//It can only work for the first time for the color picker of the datalabel font color.
			//var labelFontColor = Y.one('#AxisLabelStyle_FontColor.rdInputColorPicker');
			//new Y.LogiXML.rdInputColorPicker({configNode: labelFontColor});
		},
        ...
        rdShowColorRangePopup: function (sColumnID, sCurrentColorRanges, rdCurrentColorRangeFullRow, sAggrColorRanges, sDataType) {
            //There are already color ranges, restore them in the UI.
            var aColorRanges = [];
            if (sCurrentColorRanges.indexOf("|") > 0) {//REPDEV-24632, the format of 'sCurrentColorRanges' is ( anystring '|')*, so must contains '|'
                aColorRanges = sCurrentColorRanges.split("|");
                aColorRanges.pop();//As format, remove the last empty one.
            }
            this._rdAgCRResetColorRanges("", aColorRanges, true);
            ...
            if (eleColorRangeFullRowCheckbox) {
                eleColorRangeFullRowCheckbox.checked = (rdCurrentColorRangeFullRow == "True");
            }
            ...
            this._rdAgCRShowGroupLevelTab(sColumnID, sAggrColorRanges);
            document.getElementById("rdAgColorRangeColumnID").value = sColumnID;
            ShowElement(this.id, 'panelColorRanges', 'Show');
		},
		rdShowPercentOfTotal: function (sColumnID, sDecimalPlace) {
			if(sDecimalPlace == "") {
				document.getElementById("rdNumberOfDecimalPlaces").value = "";
			}
			else{
				document.getElementById("rdNumberOfDecimalPlaces").value = sDecimalPlace;
			}
			document.getElementById("rdAgDecimalPlaceColumnID").value = sColumnID;
			ShowElement(this.id, 'panelPercentDecimalPlaces', 'Show');
		},
        //REPDEV-24632
        _rdAgCRResetColorRanges: function (sGrpIDPrefix, aColorRangeArray, bStrictly) {
            for (i = 1; i < 11; i++) {
                var eleInputRange = document.getElementById(sGrpIDPrefix + "rdAgRangeValue" + i);
                if (bStrictly && !eleInputRange) continue;

                if (i <= aColorRangeArray.length) {
                    eleInputRange.value = aColorRangeArray[i - 1].split(":")[0];
                    this.rdSetPickerColor(sGrpIDPrefix + "rdAgRangeColor" + i, aColorRangeArray[i - 1].split(":")[1])
                } else {
                    //Set blank values and colors for rows with no data
                    eleInputRange.value = ""
                    this.rdSetPickerColor(sGrpIDPrefix + "rdAgRangeColor" + i, "")
                }
            }
            if (aColorRangeArray.length >= 6) {
                ShowElement(this.id, sGrpIDPrefix + 'rowMore', 'Hide')
                ShowElement(this.id, sGrpIDPrefix + 'rowColor6', 'Show')
                ShowElement(this.id, sGrpIDPrefix + 'rowColor7', 'Show')
                ShowElement(this.id, sGrpIDPrefix + 'rowColor8', 'Show')
                ShowElement(this.id, sGrpIDPrefix + 'rowColor9', 'Show')
                ShowElement(this.id, sGrpIDPrefix + 'rowColor10', 'Show')
            } else {
                ShowElement(this.id, sGrpIDPrefix + 'rowMore', 'Show')
                ShowElement(this.id, sGrpIDPrefix + 'rowColor6', 'Hide')
                ShowElement(this.id, sGrpIDPrefix + 'rowColor7', 'Hide')
                ShowElement(this.id, sGrpIDPrefix + 'rowColor8', 'Hide')
                ShowElement(this.id, sGrpIDPrefix + 'rowColor9', 'Hide')
                ShowElement(this.id, sGrpIDPrefix + 'rowColor10', 'Hide')
            }
        },
        _rdAgCRShowGroupLevelTab: function (sColumnID, sAggrColorRanges) {
            var eleDataCenter = document.getElementById("CRFrontDataCenter");
            if (!eleDataCenter) return; //nothing->no grouplevel.

            this.rdAgShowCRLevelTab("Record", true) //Select Record-Level

            var sDataColumn = eleDataCenter.querySelector("#AggrColumnList option[value='" + sColumnID + "']")
            if (!!!sDataColumn) { //Maybe sth error.
                //EMPTY-ALL
                this._rdAgCRBackToOneGroupCondition("", "", false);
                //HIDE MENU
                document.getElementById("rowCRLevelMenuOptions").style.display = "none";
                return;
            }
            sDataColumn = sDataColumn.textContent

            var allAggrFuncs = eleDataCenter.querySelector("#" + sDataColumn);
            if (!allAggrFuncs) {
                //EMPTY-ALL
                this._rdAgCRBackToOneGroupCondition("", "", false);
                //HIDE MENU
                ...
                var groupColorRangeArray = [];
                if (sAggrColorRanges.indexOf("|]|") > 0) {//the format for sAggrColorRanges is ( '[' anystring '|]|' )*, so indexof('|]|') must larger than 0
                    groupColorRangeArray = sAggrColorRanges.split("|]|");
                    groupColorRangeArray.pop();//As format, remove the last empty one.
                }

                var allGroups = eleDataCenter.querySelector("#CurrentGroupList");
                if (groupColorRangeArray.length == 0) {
                    this._rdAgCRBackToOneGroupCondition(allGroups.innerHTML, allAggrFuncs.innerHTML, allGroups.options.length > 1);
                } else {
                    var usedGroupColorRangeArray = [];
                    var usedGroupKeySet = new Set();
                    for (var i = 0; i < groupColorRangeArray.length; i++) {
                        var aGroupColorRange = groupColorRangeArray[i].substr(1).split("|");
                        if (aGroupColorRange.length <= 1) continue;

                        var theGrpSettings = aGroupColorRange[0].split(":");
                        var selGrpIdx = -1;
                        for (var j = 0; j < allGroups.options.length; j++) {
                            if (usedGroupKeySet.has(theGrpSettings[0])) continue;
                            if (allGroups.options[j].value === theGrpSettings[0]) {
                                selGrpIdx = j; break;
                            }
                        }
                        if (selGrpIdx < 0) continue;//NO SUCH GROUP NOW.
                        var selAggrFuncIdx = -1;
                        for (var j = 0; j < allAggrFuncs.options.length; j++) {
                            if (allAggrFuncs.options[j].value === theGrpSettings[1]) {
                                selAggrFuncIdx = j; break;
                            }
                        }
                        if (selAggrFuncIdx < 0) continue;//NO SUCH AggrFunc NOW.

                        usedGroupKeySet.add(theGrpSettings[0]);
                        usedGroupColorRangeArray.push(aGroupColorRange)
                    }
                    var groupContainer = document.getElementById("GroupConditionContainer");
                    var minCount = Math.min(usedGroupColorRangeArray.length, groupContainer.children.length);

                    usedGroupKeySet.clear();
                    if (minCount == 0) {
                        this._rdAgCRBackToOneGroupCondition(allGroups.innerHTML, allAggrFuncs.innerHTML, allGroups.options.length > 1)
                    } else {
                        for (var aIdx = 0; aIdx < minCount; aIdx++) {
                            usedGroupKeySet.add(usedGroupColorRangeArray[aIdx][0].split(":")[0]);
                        }
                        var sGrpIDPrefixes = ""
                        for (var aIdx = 0; aIdx < minCount; aIdx++) {
                            var sGrpIDPrefix = this._rdAgCRGetGroupCondtionIDPrefix(groupContainer.children[aIdx]);
                            var grpSelect = document.getElementById(sGrpIDPrefix + "rdAgRangeAggrGroup");
                            grpSelect.innerHTML = allGroups.innerHTML;

                            var delOpts = [];
                            var sSelectedGrpKey = usedGroupColorRangeArray[aIdx][0].split(":")[0];
                            for (var i = 0; i < grpSelect.options.length; i++) {
                                if (grpSelect.options[i].value !== sSelectedGrpKey
                                    && usedGroupKeySet.has(grpSelect.options[i].value))
                                    delOpts.push(grpSelect.options[i]);
                            }
                            delOpts.forEach(function (delOpt) { delOpt.parentNode.removeChild(delOpt); });

                            document.getElementById(sGrpIDPrefix + "rdAgRangeAggrFunc").innerHTML = allAggrFuncs.innerHTML;

                            this._rgAgCRFillGroupCondition(sGrpIDPrefix, usedGroupColorRangeArray[aIdx]);

                            if (aIdx == 0)
                                sGrpIDPrefixes = sGrpIDPrefix;
                            else
                                sGrpIDPrefixes = sGrpIDPrefixes + "|" + sGrpIDPrefix;
                        }
                        usedGroupKeySet.clear();
                        document.getElementById("rdAgCRGroupConditionIDPrefixes").value = sGrpIDPrefixes;

                        if (minCount == usedGroupColorRangeArray.length) {
                            for (var aIdx = groupContainer.children.length - 1; aIdx >= minCount; aIdx--) {
                                groupContainer.removeChild(groupContainer.children[aIdx]);
                            }
                            if (minCount < allGroups.options.length) {
                                //Enable add-GroupCondition
                                document.getElementById(this._rdAgCRGetGroupCondtionIDPrefix(groupContainer.lastElementChild) + "lblAddGroupCondition").style.display = "";
                            }
                        } else {
                            for (var aIdx = minCount; aIdx < usedGroupColorRangeArray.length; aIdx++) {
                                this.rdAgCRAddGroup(usedGroupColorRangeArray[aIdx][0].split(":")[0])
                                this._rgAgCRFillGroupCondition(this._rdAgCRGetGroupCondtionIDPrefix(groupContainer.lastElementChild), usedGroupColorRangeArray[aIdx]);
                            }
                        }
                    }
                }
                //SHOW MENU
                document.getElementById("rowCRLevelMenuOptions").style.display = "";
            }
        },
        _rdAgCRBackToOneGroupCondition: function (grpOptionsHtml, aggrFuncOptionHtml, enableAddGC) {
            var groupContainer = document.getElementById("GroupConditionContainer")
            var sGrpIDPrefix = this._rdAgCRGetGroupCondtionIDPrefix(groupContainer.firstElementChild);

            document.getElementById("rdAgCRGroupConditionIDPrefixes").value = sGrpIDPrefix;

            document.getElementById(sGrpIDPrefix + "rdAgRangeAggrGroup").innerHTML = grpOptionsHtml;
            document.getElementById(sGrpIDPrefix + "rdAgRangeAggrFunc").innerHTML = aggrFuncOptionHtml;
            document.getElementById(sGrpIDPrefix + "rdAgRangeFullRow").checked = false;
            this._rdAgCRResetColorRanges(sGrpIDPrefix, []);

            document.getElementById(sGrpIDPrefix + "lblAddGroupCondition").style.display = (enableAddGC ? "" : "none");

            //Remove other div
            for (var i = groupContainer.children.length - 1; i >= 1; i--)
                groupContainer.removeChild(groupContainer.children[i]);
        },
        _rdAgCRGetGroupCondtionIDPrefix: function (aGroupCondition) { return aGroupCondition.id.split("_")[0] + "_"; },
        _rgAgCRFillGroupCondition: function (sGrpIDPrefix, aGroupColorRange) {
            var theGrpSettings = aGroupColorRange.shift().split(":");
            var selectNode = document.getElementById(sGrpIDPrefix + "rdAgRangeAggrGroup");
            for (var j = 0; j < selectNode.options.length; j++) {
                if (selectNode.options[j].value === theGrpSettings[0]) {
                    selectNode.selectedIndex = j; break;
                }
            }
            selectNode = document.getElementById(sGrpIDPrefix + "rdAgRangeAggrFunc")
            for (var j = 0; j < selectNode.options.length; j++) {
                if (selectNode.options[j].value === theGrpSettings[1]) {
                    selectNode.selectedIndex = j; break;
                }
            }
            //fullRowChkbox
            document.getElementById(sGrpIDPrefix + "rdAgRangeFullRow").checked = (theGrpSettings[2] == "True");

            //Value Cells
            this._rdAgCRResetColorRanges(sGrpIDPrefix, aGroupColorRange) //NOTE: there is 'shift()' before calling
        },

        rdAgShowCRLevelTab: function (sTabName, bCheckForReset) {
            var curOpenTabName = document.getElementById('rdAgCurrentOpenCRLevel').value
            if (!bCheckForReset && curOpenTabName == sTabName) return;

            document.getElementById('rdAgCurrentOpenCRLevel').value = sTabName;

            var eleSelectedTab = document.getElementById('lblHeadingCRLevel' + sTabName);

            this.rdSetClassNameById('lblHeadingCRLevelRecord', 'rdAgCommandIdle');
            this.rdSetClassNameById('lblHeadingCRLevelGroup', 'rdAgCommandIdle');
            this.rdSetDisplayById('rowCRLevelRecord', 'none');
            this.rdSetDisplayById('rowCRLevelGroup', 'none');

            if (eleSelectedTab) {
                eleSelectedTab.className = 'rdAgCommandHighlight';
                var eleSelectedRow = document.getElementById('rowCRLevel' + sTabName);
                eleSelectedRow.style.display = '';
                if (!bCheckForReset && eleSelectedRow.firstChild)   // Avoid flicker/fading effect when Paged/Sorted/Postbacks.
                    rdFadeElementIn(eleSelectedRow.firstChild, 400);    //#11723, #17294 tr does not handle transition well.
            }
        },
        rdAgCRDeleteGroup: function (sGrpIDPrefix) {
            var groupDiv = document.getElementById(sGrpIDPrefix + "GroupCondition");
            if (!groupDiv) return;

            var groupContainer = groupDiv.parentElement;
            if (groupContainer.childElementCount == 1) {
                //Just Clear Values
                document.getElementById(sGrpIDPrefix + "rdAgRangeAggrGroup").innerHTML = document.querySelector("#CRFrontDataCenter #CurrentGroupList").innerHTML;
                document.getElementById(sGrpIDPrefix + "rdAgRangeAggrFunc").selectedIndex = 0;
                document.getElementById(sGrpIDPrefix + "rdAgRangeFullRow").checked = false;
                this._rdAgCRResetColorRanges(sGrpIDPrefix, []);
            } else {
                var eleGrp = document.getElementById(sGrpIDPrefix + "rdAgRangeAggrGroup")
                var selOpt = eleGrp.options[ eleGrp.selectedIndex ];
                groupContainer.removeChild(groupDiv);

                var eleGrpIDPrefixes = document.getElementById("rdAgCRGroupConditionIDPrefixes");
                if (eleGrpIDPrefixes.value.startsWith(sGrpIDPrefix))
                    eleGrpIDPrefixes.value = eleGrpIDPrefixes.value.replace(sGrpIDPrefix + "|", "");
                else
                    eleGrpIDPrefixes.value = eleGrpIDPrefixes.value.replace("|" + sGrpIDPrefix, "");

                //Retore options to other groups.
                for (var i = 0; i < groupContainer.children.length; i++) {
                    groupDiv = groupContainer.children.item(i);
                    sGrpIDPrefix = this._rdAgCRGetGroupCondtionIDPrefix(groupDiv);
                    var groupSelect = document.getElementById(sGrpIDPrefix + "rdAgRangeAggrGroup");
                    var newOpt = document.createElement("option")
                    newOpt.value = selOpt.value;
					newOpt.innerHTML = selOpt.innerHTML;
                    groupSelect.appendChild(newOpt);
                }

                //Enable Add GroupCondition.
                document.getElementById(sGrpIDPrefix + "lblAddGroupCondition").style.display = "";
            }
        },
        rdAgCRAddGroup: function (sPreSelectedGrpKey) {
        	var groupContainer = document.getElementById("GroupConditionContainer");
        	var lastGroupDiv = groupContainer.lastElementChild;
        
        	var lastGrpIDPrefix = this._rdAgCRGetGroupCondtionIDPrefix(lastGroupDiv);
        	//HIDE Last's Add-GroupCondition
        	lastGroupDiv.querySelector("#" + lastGrpIDPrefix + "lblAddGroupCondition").style.display = "none";
        
        	var newGrpIDPrefix = "G" + (parseInt(lastGroupDiv.id.split("_")[0].substring(1)) + 1) + "_";
        	var newGroupDiv = document.createElement(lastGroupDiv.tagName);
        	newGroupDiv.id = newGrpIDPrefix + "GroupCondition";
        	newGroupDiv.style.display = "none";//1)Reduce the cost in IE, 2)Reduce page flicker.

		if (lastGroupDiv.innerHTML.replaceAll)
			newGroupDiv.innerHTML = lastGroupDiv.innerHTML.replaceAll(lastGrpIDPrefix, newGrpIDPrefix);
		else
			newGroupDiv.innerHTML = lastGroupDiv.innerHTML.replace(new RegExp(lastGrpIDPrefix, "gm"), newGrpIDPrefix); //FOR IE or other (lower) Browser

        
        	var newGrpSelect = newGroupDiv.querySelector("#" + newGrpIDPrefix + "rdAgRangeAggrGroup");
        	var lastGrpSelect = lastGroupDiv.querySelector("#" + lastGrpIDPrefix + "rdAgRangeAggrGroup");
        	//Remove Other GroupDiv selected Option
        	for (var i = 0; i < groupContainer.children.length; i++) {
        		var groupDiv = groupContainer.children.item(i);
        		var sGroupIDFlag = this._rdAgCRGetGroupCondtionIDPrefix(groupDiv);
			var groupSelect = groupDiv.querySelector("#" + sGroupIDFlag + "rdAgRangeAggrGroup")
			this._rdAgCRRemoveGroupOption(newGrpSelect, groupSelect.options[groupSelect.selectedIndex])
        	}
        	if (sPreSelectedGrpKey) {
        		for (var i = 0; i < newGrpSelect.options.length; i++) {
        			if (newGrpSelect.options[i].value === sPreSelectedGrpKey) {
        				newGrpSelect.selectedIndex = i;
        				newGrpSelect.lastSelectedOption = newGrpSelect.options[i];
        				break;
        			}
        		}
        	}
        
        	//Remove/restore rdInputColorPicker selected-status (copyied from lastGrp).
        	for (var i = 1; i < 11; i++) {
        		var targetTempID = newGrpIDPrefix + "rdAgRangeColor" + i;
        		var opNode = newGroupDiv.querySelector("img#selectedColor_" + targetTempID);
        		if (opNode.parentNode.id !== "PopupPanelContent") {
        			opNode.parentNode.classList.remove("rdImageColorSelected")
        			opNode.parentNode.removeChild(opNode)
        			
        			var divNode = newGroupDiv.querySelector("#rdPopupPanelTable_ppColors_" + targetTempID + " div#PopupPanelContent")
        			divNode.appendChild(opNode)
        		}
        		opNode = newGroupDiv.querySelectorAll("#ppTiles_" + targetTempID + ", #ppSaturation_" + targetTempID);
        		for (var j = 0; j < opNode.length; j++) {//opNode.length must be 2
        			opNode.item(j).innerHTML = ""
				}
				opNode = newGroupDiv.querySelector("input#" + targetTempID)
				opNode.removeAttribute("value")
        	}
        	
        	newGroupDiv.querySelector("#"+newGrpIDPrefix + "rdAgRangeFullRow").checked = false;
        	
        	//Enable New's Add-GroupCondition
        	if (newGrpSelect.options.length > 1) {
        		newGroupDiv.querySelector("#" + newGrpIDPrefix + "lblAddGroupCondition").style.display = ""
        	}
        	//Remove Other GroupDiv's Option-choice
        	for (var i = 0; i < groupContainer.children.length; i++) {
        		var groupDiv = groupContainer.children.item(i);
        		var sGroupIDFlag = this._rdAgCRGetGroupCondtionIDPrefix(groupDiv);
        		var groupSelect = groupDiv.querySelector("#" + sGroupIDFlag + "rdAgRangeAggrGroup")
			this._rdAgCRRemoveGroupOption(groupSelect, newGrpSelect.options[newGrpSelect.selectedIndex])
        	}

        	groupContainer.appendChild(newGroupDiv);
        	var eleGrpIDPrefixes = document.getElementById("rdAgCRGroupConditionIDPrefixes");
        	eleGrpIDPrefixes.value = eleGrpIDPrefixes.value + "|" + newGrpIDPrefix;
        	
        	//rdInputColorPicker regist
        	Y.all("#" + newGroupDiv.id + " .rdInputColorPicker").each(function (node) {
        		element = node.getData("rdInputColorPicker");
        		if (!element) {element = new Y.LogiXML.rdInputColorPicker({configNode: node});}
        	});

        	this._rdAgCRResetColorRanges(newGrpIDPrefix, []);//MUST PUT HERE, document required
        	newGroupDiv.style.display = "";//it is hidden by above reasons
        },
        rdAgCRGroupChoiceChanged: function (sGrpIDPrefix) {
        	var groupContainer = document.getElementById("GroupConditionContainer");
        	var curSelect = document.getElementById(sGrpIDPrefix + "rdAgRangeAggrGroup")
        	
        	var lastSelOpt = curSelect.lastSelectedOption;
        	if (!lastSelOpt) lastSelOpt = curSelect.options[0];
        	
        	var curSelOpt = curSelect.options[curSelect.selectedIndex];
        	if (curSelOpt == lastSelOpt) return;
        	curSelect.lastSelectedOption = curSelOpt;
        
        	for (var i = 0; i < groupContainer.children.length; i++) {
        		var groupDiv = groupContainer.children.item(i);
        		var sGroupIDFlag = this._rdAgCRGetGroupCondtionIDPrefix(groupDiv);
        		if (sGroupIDFlag === sGrpIDPrefix) continue;
        		var groupSelect = groupDiv.querySelector("#" + sGroupIDFlag + "rdAgRangeAggrGroup")
        		this._rdAgCRRemoveGroupOption(groupSelect, curSelOpt);
        		
        		var newOpt = document.createElement("option")
        		newOpt.value = lastSelOpt.value;
        		newOpt.innerHTML = lastSelOpt.innerHTML;
        		groupSelect.appendChild(newOpt);
        	}
        },
        _rdAgCRRemoveGroupOption: function (opSelectNode, toRemovedOpt) {
        	for (var i = 0; i < opSelectNode.options.length; i++) {
        		var curOpt = opSelectNode.options[i];
        		if (curOpt.value == toRemovedOpt.value) {
...
function rdchangeList(rdEleId, aNewAggrList, aLabel, sDataColumnType, aAggrGroupLabel, aAggrGroupLabelClass, sCurrentColumnToken) {
    ...
    rdemptyList(rdEleId);    
	rdfillList(rdEleId, aNewAggrList, aLabel, sDataColumnType, sSelectedValue, aAggrGroupLabel, aAggrGroupLabelClass, sCurrentColumnToken);
}
...
function rdfillList(rdEleId, arr, aLabel, sDataColumnType, sSelectedValue, arrGroupLabel, arrGroupLabelClass, sCurrentColumnToken) {
    ...
    if (sDataColumnType.toLowerCase() == "boolean" ||
        sDataColumnType.toLowerCase() == "text" ||
	...
	if (rdAgIsShowCustAgg()) {
        var ddlAggrCol = document.getElementById("rdAgAggrColumn");
        if (ddlAggrCol && ddlAggrCol.selectedIndex >= 0) {
			var custAggs = rdAgGetCustomAggregates(ddlAggrCol.options[ddlAggrCol.selectedIndex].value, sDataColumnType, sCurrentColumnToken);
...
function rdAgIsShowCustAgg() {
	var hdn = document.getElementById("rdAgShowCustAgg");
	if (!hdn || !hdn.value)
		return false;
...
function rdAgGetCustomAggregates(sColumnID, sDataType, sCurrentColumnToken) {
    var dt = document.getElementById("dtCustAggs");
    var custAggs = [];
	//var sCurrentColumnToken = LogiXML.AnalysisGrid.CURRENT_COLUMN_TOKEN;
	if (sCurrentColumnToken == undefined) {
		sCurrentColumnToken = LogiXML.AnalysisGrid.CURRENT_COLUMN_TOKEN;
    }

	for (var i = 1; i < dt.rows.length; i++) {
		//REPDEV-27884. If there is no data, a row for no data caption may be built sometimes, this row need be ignored here.
		//REPDEV-28123, Datatable "dtCustAggs" don't support no data caption, so don't need check no data row
		//if (dt.rows[i].id == "rdNoDataDatatableRowId") {
		//	continue;
		//}
...
function rdAgAddCustomAggregatesToHeaders() {
    ...
    for (var i = 0; i < headers.length; i++) {
        ...
        if (!aggPopup)
            continue;
        //REPDEV-29968 Custom option below Aggregate doesn't display when click Next/Last page button
        //when the user click the Next/Last page button, the suffix will be "_Row21", "_Row41".
        //It isn't always the "_Row1", so get it dynamically.
        suffix = aggPopup.id;
        var idx = suffix.lastIndexOf("_Row");
		if (idx > -1) {
        ...
        if (custAggs) {
            for (var j = 0; j < custAggs.length; j++) {
                ...
                //REPDEV-29845, has it been added?
                var added = false;
                for (var k = 0; k < targetUL.children.length; k++) {
                    var linkid = targetUL.childNodes[k].childNodes[0].id;
                    if (linkid == newLink.id) {
                        added = true;
                        break;
                    }
                }
        ...
        var ddlDataColumn = document.getElementById("rdAgAggrColumn");
		var popupCaption = rdAgCustAggShowPopupCaption();
		var added = isAdded(targetUL, "custom")
		if (rdAgIsShowCustAgg() && !added && ddlDataColumn) {
            var dcIdx = -1;
            ...
            if (dcIdx >= 0) {
                ...
                newLink.id = "custom"+LogiXML.getGuid();

                newLink.setAttribute("onclick", "rdAgCustAggCustomHeaderClick(" + dcIdx + ");");

				newLink.childNodes[0].innerText = popupCaption;
                targetUL.appendChild(newOpt);
            }
		}

		var aggFilterPanel = document.getElementById("rdAggregateAnalysisFilterPopup");
		if (aggFilterPanel && !isAdded(targetUL, "AggFilter"))  {
			// REPDEV-26524 Add "(Filter)" option
			newOpt = newOpts[0].cloneNode(true);
			newLink = newOpt.childNodes[0];
			popupCaption = rdAggAnalysisFilterShowPopupCaption();
			newLink.id = "AggFilter"+LogiXML.getGuid();

			newLink.setAttribute("onclick", "ShowElement(null, 'rdAggregateAnalysisFilterPopup', 'Show');");
...
function isAdded(targetUL, popupCaption) {
	var lis = targetUL.getElementsByTagName("li")
	var li
	for (var j = 0; j < lis.length; j++) {
		li = lis[j];
		var link = li.childNodes[0];
		if (link.getAttribute("id").startsWith(popupCaption)) {
			return true;
        }
	}
	return false;
...
function rdAggAnalysisFilterShowPopupCaption() {
	return document.getElementById("rdAggregateAnalysisFilterShowPopupCaption").innerText;
}
...
function rdCalcUpdateUi(sCommand, replaceButton) {
	resetErrorMessage();
	var sCalcName = document.getElementById("rdAgCalcName").value;
	if (sCalcName == undefined || sCalcName == "") {
		ShowElement(null, "divCalcError-BlankName", "Show", null)
		return false;
	}

	var sCalcFormula = document.getElementById("rdAgCalcFormula").value;
	if (sCalcFormula == undefined || sCalcFormula == "") {
		ShowElement(null, "divCalcError-BlankFormula", "Show", null)
		return false;
	}

	var replaceAction = false;
	if (sCommand == "CalcReplace") {
		replaceAction = true;
		var rowNum = replaceButton.getAttribute("id").slice("actCalcReplace_Row".length)
	} 
	var tr = document.getElementById("dtCalcColumns").getElementsByTagName("tr")
	for (var i = 0; i < tr.length; i++) {
		//REPDEV-27884. If there is no data, a row for no data caption may be built sometimes, this row need be ignored here.
		if (tr[i].id == "rdNoDataDatatableRowId") {
			continue;
        }
		if (sCalcName == document.getElementById("lblCalcColumnName_Row" + (i + 1)).innerHTML) {
			if (!replaceAction || (replaceAction && rowNum != i + 1)) {
