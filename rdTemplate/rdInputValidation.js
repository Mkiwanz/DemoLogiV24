...
function rdValidateEmailAddress(sFieldId, sErrorMsg, sErrorClass) {
	...
	if (eleInput) {
		if (rdIsInputVisible(sFieldId)) {
			...
			//In the mail address specifiction, these chararcters !#$%&'*+-/=?^`{|}~ are allowed on the local part of the email address.
			//As we can't test the chararcters *=| {}/? with an appropriate SMTP to check whether the email address which holds these special characters can be sent or received successfully, 
			//same as before we set the email address as invalidate when the user input these characters in the email address.
			//In other words, these chararcters !#$%&'+-^`~ are allowed on the local part of the email address.
...
function rdConfirmAndValidateActionJavascript(e, sConfirmationMsg, bValidate, onOk, onCancel) {
    if (onCancel)
    ...
    return LogiXML.confirm(sConfirmationMsg, onOk, onCancel);
}
