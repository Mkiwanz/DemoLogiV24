...
{
    var newCookie = sName + "=" + encodeURI(sValue) + ((sExpires) ? "; expires=" + sExpires : "") + ((sPath) ? "; path=" + sPath : "") + ";";
    var sameSite = LogiXML.CookieSameSite();

    if (sameSite) {
        document.cookie = newCookie + sameSite;

        if (getCookie2(sName) != sValue) {
...
function setCookie2(name, value, expires, path, domain, secure)
{
    ...
    var newCookie = name + "=" + escape(value) +
        ...
        ";";
    var sameSite = (LogiXML.CookieSameSite() || ((secure) ? "secure" : ""));

    if (sameSite) {
        document.cookie = newCookie + sameSite;

        if (getCookie2(name) != value) {
            document.cookie = newCookie; // IE
        }
    } else {
        document.cookie = newCookie;
